...
import { createSecureToken, EMAIL_VERIFICATION_TTL_MS, expiresAtFromNow, getAppUrl, getPublicApiBaseUrl, PASSWORD_RESET_TTL_MS, } from "../utils/secureToken.js";
...
export async function issueEmailVerification(params) {
    ...
    try {
        ...
        if (isLocal) {
            console.warn(`[email] Verification email was not delivered to ${params.email}.` +
...
export async function resendVerificationEmail(email, apiBaseUrl) {
...
export async function changePasswordForUser(params) {
    ...
    if (!user.password) {
        throw new AuthEmailError("This account uses Google sign-in. Continue with Google, or set a password after linking email login.");
    }
...
//# sourceMappingURL=authEmail.service.js.map