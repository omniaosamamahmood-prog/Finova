...
import type { UserPlan } from "../utils/plan.js";
...
type PremiumFulfillmentResult =
  | { status: "upgraded"; user: { id: string; plan: "PREMIUM" } }
  | { status: "already_premium"; user: { id: string; plan: "PREMIUM" } }
  | { status: "ignored"; reason: "not_paid" };

export async function fulfillPremiumCheckoutFromSession(
...
export async function createPremiumCheckoutSession(userId: string) {
  const user = await prisma.user.findUnique({
    where: { id: userId },
