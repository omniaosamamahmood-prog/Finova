...
export async function buildFinancialContext(
  ...
  for (const transaction of monthTransactions) {
    ...
    expenseByCategoryId.set(
      ...
      (expenseByCategoryId.get(transaction.categoryId) ?? 0) + transaction.amount
    );
  }
  ...
  const budgetContext = budgets.map((budget) => {
    ...
    const progress =
      budget.amount > 0 ? roundPercent((spent / budget.amount) * 100) : 0;
  ...
  const upcomingExpenseTotalThisMonth = roundMoney(
    recurringDueThisMonth.reduce((sum, item) => sum + item.amount, 0)
  );
