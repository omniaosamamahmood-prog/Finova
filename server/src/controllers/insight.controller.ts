...
export async function getFinancialInsights(
  ...
  try {
    ...
    if (monthTransactions.length === 0) {
      insights.push(
        createInsight("info", "noTransactions", "lightbulb")
      );
    }
    ...
    if (monthExpenseGroups.length > 0) {
      ...
      const topExpense = [...monthExpenseGroups].sort(
      ...
      if (categoryName && (topExpense._sum.amount ?? 0) > 0) {
        insights.push(
          createInsight("info", "highestSpending", "pie-chart", {
    ...
    const spentByCategoryId = new Map(
      allExpenseGroups.map((group) => [group.categoryId, group._sum.amount ?? 0])
    );
    ...
    for (const budget of budgets) {
      ...
      if (spent > budget.amount) {
        insights.push(
          createInsight("danger", "budgetExceeded", "x-circle", {
            category: categoryName,
      ...
      if (progress >= 80) {
        insights.push(
          createInsight("warning", "budgetWarning", "alert-triangle", {
            category: categoryName,
          })
        );
      }
    ...
    for (const goal of goals) {
      ...
      if (progress >= 100) {
        insights.push(
          createInsight("success", "goalComplete", "trophy", {
            name: goal.name,
          })
        );
        continue;
      ...
      if (progress >= 75) {
        insights.push(
          createInsight("success", "goalClose", "target", {
            name: goal.name,
          })
        );
      }
    ...
    if (recurringExpensesDue.length > 0) {
      const recurringTotal = recurringExpensesDue.reduce(
        (total, item) => total + item.amount,
        0
      );
      insights.push(
        createInsight("info", "recurringDue", "repeat", {
    ...
    if (monthIncome > 0 || monthExpense > 0) {
      ...
      if (savings > 0 && monthIncome > 0) {
        insights.push(
          createInsight("success", "savingsPositive", "piggy-bank", {
            percent: roundPercent((savings / monthIncome) * 100),
          })
        );
      } else if (monthExpense > monthIncome) {
        insights.push(
          createInsight("warning", "savingsNegative", "trending-down")
        );
      }
    }
