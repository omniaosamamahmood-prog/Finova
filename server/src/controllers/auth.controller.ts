...
function friendlyError(res: Response, error: unknown) {
...
export async function verifyEmail(req: Request, res: Response) {
  ...
  const redirectStatus =
    status === "success" ? "success" : status === "expired" ? "expired" : "failed";
  ...
  try {
    ...
    return res.redirect(
      `${appUrl}/verify-email?status=${encodeURIComponent(redirectStatus)}`
    );
  } catch {
